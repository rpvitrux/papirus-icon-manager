...
class PapirusIconManager:
    ...
    def _detect_app_type(self, app_info):
        ...
        text_to_check = (
            f"{app_info['name']} {app_info['comment']} {app_info['exec']}".lower()
        )
                ...
                for category in categories:
                    for ext in extensions:
                        potential_path = os.path.join(
                            papirus_path, size, category, f"{icon_base}{ext}"
                        )
        ...
        for package_type, paths in self.desktop_paths.items():
            for desktop_dir in paths:
                ...
                for desktop_file in glob.glob(os.path.join(desktop_dir, "*.desktop")):
                    ...
                    if self.debug_mode:
                        self.debug_log(
                            f"Processing app '{app_info['name']}' with icon '{app_info['icon']}'"
                        )
                    ...
                    if self.debug_mode:
                        self.debug_log(
                            f"Final result - is_using_papirus: {is_using_papirus}"
                        )
                        self.info_log("")

                    if is_using_papirus:
                        papirus_apps.append(
                            {"app_info": app_info, "resolved_path": icon_path}
                        )
                        if self.debug_mode:
                            self.info_log(
                                f"  ✓ {app_info['name']} ({app_info['package_type']}) - Using Papirus"
                            )
                            self.info_log(
                                f"    Icon: {app_info['icon']} → {os.path.basename(icon_path) if icon_path else 'unknown'}"
                            )
                        ...
                        papirus_alternatives = self._suggest_papirus_alternatives(
                            app_info
                        )
                        ...
                        if not icon_path:
                            ...
                            status_msg = (
                                f"Using {self._get_theme_name_from_path(icon_path)}"
                            )

                        if self.debug_mode:
                            self.info_log(
                                f"  ✗ {app_info['name']} ({app_info['package_type']}) - NOT using Papirus"
                            )
                            ...
                            if papirus_alternatives:
                                self.info_log(
                                    f"    Papirus alternatives: {', '.join(papirus_alternatives[:3])}"
                                )
                            else:
                            ...
                            if papirus_alternatives:
                                self.info_log(
                                    f"    Suggested fix: Use '{papirus_alternatives[0]}'"
                                )
        ...
        try:
            ...
            if needs_sudo:
                ...
                override_file = os.path.join(
                    user_apps_dir, os.path.basename(desktop_file_path)
                )
            ...
            if "Desktop Entry" in config:
                ...
                self.debug_log(
                    f"Updated icon in {desktop_file_path} from '{old_icon}' to '{new_icon_name}'"
                )
                return True
...
Examples:
  ...
  python3 papirus_manager.py --scan --debug
    ...
    parser.add_argument(
        "--fix", action="store_true", help="Apply fixes for missing Papirus icons"
    )
    parser.add_argument(
        "--auto",
        action="store_true",
